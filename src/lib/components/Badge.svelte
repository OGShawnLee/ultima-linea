...
<a
		class="button {featured ? "bg-accent-light text-summit hover:bg-accent-light/75 dark:(bg-accent text-summit-light hover:bg-accent/75) font-semibold" : "button--background"} h-8 px-4 rounded-full"
	href={region ? '/articles/region/' + region.toLowerCase() : href}
>
	{#if icon}
